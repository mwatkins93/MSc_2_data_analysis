...
#### 3.04.1 - Mean DOC ----
mean_base_model <- lm(mean.doc ~ Group + conifer_st + drainage_st + elev_st + slope_st + wetland_st, data = standard_doc_tbl)
...
sc1_base_model <- lm(doc.s1 ~ Group + conifer_st + drainage_st + elev_st + slope_st + wetland_st, data = standard_doc_tbl)
...
sc2_base_model <- lm(doc.s2 ~ Group + conifer_st + drainage_st + elev_st + slope_st + wetland_st, data = standard_doc_tbl)
...
sc3_base_model <- lm(doc.s3 ~ Group + conifer_st + drainage_st + elev_st + slope_st + wetland_st, data = doc_s3_sub)
...
sc4_base_model <- lm(doc.s4 ~ Group + conifer_st + drainage_st + elev_st + slope_st + wetland_st, data = doc_s4_sub)
...
sc5_base_model <- lm(doc.s5 ~ Group + conifer_st + drainage_st + elev_st + slope_st + wetland_st, data = doc_s5_sub)
...
sc6_base_model <- lm(doc.s6 ~ Group + conifer_st + drainage_st + elev_st + slope_st + wetland_st, data = standard_doc_tbl)
...
mean_base_model_plot <- dwplot(base_model_avgs) %>%
  ...
  theme_bw() +
  labs(title = "Average multiple regression coefficients - slope + wetland") +
