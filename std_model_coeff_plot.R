...
#### 3.04.1 - Mean DOC ----
mean_base_model <- lm(mean.doc ~ Group + conifer_st + drainage_st + elev_st + open_wat_st, data = standard_doc_tbl)
...
sc1_base_model <- lm(doc.s1 ~ Group + conifer_st + drainage_st + elev_st + open_wat_st, data = standard_doc_tbl)
...
sc2_base_model <- lm(doc.s2 ~ Group + conifer_st + drainage_st + elev_st + open_wat_st, data = standard_doc_tbl)
...
sc6_base_model <- lm(doc.s6 ~ Group + conifer_st + drainage_st + elev_st + open_wat_st, data = standard_doc_tbl)
...
mean_base_model_plot <- dwplot(base_model_avgs) %>%
  ...
  theme_bw() +
  labs(title = "Average multiple regression coefficients - open water only") +
