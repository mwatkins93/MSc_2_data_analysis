...
#### 3.04.1 - Mean DOC ----
mean_base_model <- lm(mean.doc ~ Group + conifer_st + drainage_st + elev_st + open_wat_st + wetland_st + slope_st, data = doc_tbl_no_c3)
...
mean_base_model_avg <- model.avg(subset(mean_base_model_tbl, delta <= 2.3, recalc.weights=FALSE), fit = TRUE)

#### 3.04.2 - Sample campaign 1 ----

sc1_base_model <- lm(doc.s1 ~ Group + conifer_st + drainage_st + elev_st + open_wat_st + wetland_st + slope_st, data = doc_tbl_no_c3)

sc1_base_table <- dredge(sc1_base_model, rank = "AICc")

sc1_base_model_avg <- model.avg(subset(sc1_base_table, delta <= 2.2, recalc.weights = FALSE), fit = TRUE)

#### 3.04.3 - Sample campaign 2 ----

sc2_base_model <- lm(doc.s2 ~ Group + conifer_st + drainage_st + elev_st + open_wat_st + wetland_st + slope_st, data = doc_tbl_no_c3)
...
doc_s3_sub <- doc_tbl_no_c3[-3, c(1:53, 56)]
...
doc_s4_sub <- doc_tbl_no_c3[c(-3, -6), c(1:53, 57)]
...
doc_s5_sub <- doc_tbl_no_c3[-3, c(1:53, 58)]
...
sc6_base_model <- lm(doc.s6 ~ Group + conifer_st + drainage_st + elev_st + open_wat_st + wetland_st + slope_st, data = doc_tbl_no_c3)
...
mean_base_model_plot <- dwplot(base_model_avgs) %>%
  ...
  theme_bw() +
  labs(title = "Average multiple regression coefficients - no C3") +
