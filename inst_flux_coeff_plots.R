...
if1_base_model <- lm(inst.flux.1 ~ Group + conifer_st + drainage_st + elev_st + open_wat_st, data = no_c3)
...
inst_flux_base_model_plot <- dwplot(inst_flux_base_model_avgs) %>%
  ...
  theme_bw() +
  labs(title = "Averaged instantaneous flux coefficients - open water only") +
