...
if1_base_model <- lm(inst.flux.1 ~ Group + conifer_st + insect20_st + drainage_st + elev_st + wetland_st + open_wat_st + slope_st, data = no_c3)
...
if2_base_model <- lm(inst.flux.2 ~ Group + conifer_st + insect20_st + drainage_st + elev_st + wetland_st + open_wat_st + slope_st, data = if2_sub)
...
if3_base_model <- lm(inst.flux.3 ~ Group + conifer_st + insect20_st + drainage_st + elev_st + wetland_st + open_wat_st + slope_st, data = if3_sub)
...
if4_base_model <- lm(inst.flux.4 ~ Group + conifer_st + insect20_st + drainage_st + elev_st + wetland_st + open_wat_st + slope_st, data = if4_sub)
...
if5_base_model <- lm(inst.flux.5 ~ Group + conifer_st + insect20_st + drainage_st + elev_st + wetland_st + open_wat_st + slope_st, data = if5_sub)
...
if6_base_model <- lm(inst.flux.6 ~ Group + conifer_st + insect20_st + drainage_st + elev_st + wetland_st + open_wat_st + slope_st, data = if6_sub)
...
inst_flux_base_model_plot <- dwplot(inst_flux_base_model_avgs) %>%
  ...
  theme_bw() +
  labs(title = "Averaged instantaneous flux coefficients - 20-year insect") +
